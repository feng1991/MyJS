...
//短时间，形如 (13:04:06),最原始match里面的内容(/^(\d{1,2})(:)?(\d{1,2})\2(\d{1,2})$/),现在进行更新
function isTime(str) {
	var a = str.match(/^([01][0-9]|2[0-3]):([0-5][0-9]):([0-5][0-9])$/);
	if (a == null) {
		return false
	}
...
function isRate(str) {
	var regStr1 = /^0\.[0-9]+$/;
	var regStr2 = /^[1-9][0-9]*.[0-9]+$/;
	var regStr3 = /^[1-9][0-9]*$/;
	// return regStr1.test(str);
	return regStr1.test(str) || regStr2.test(str) || regStr3.test(str);
}
...
function isMobileOrEmail(str) {
	var isemail = str.match(/([a-zA-Z0-9]+[_|\_|\.]?)*[a-zA-Z0-9]+@([a-zA-Z0-9]+[_|\_|\.\-]?)*[a-zA-Z0-9]+\.[a-zA-Z]{2,3}$/);
	var ismobile = str.match(/^0?1[3|4|5|8][0-9]\d{8}$/);
	if (!isemail && !ismobile) {
		return "邮箱或手机格式有误";
	}
	var nametype = isemail ? 'email' : 'mobile';
